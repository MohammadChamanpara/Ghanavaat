...
namespace APMTools
{
    public static class ExtensionMethods
    {
        ...
        public static int Round(this int number, int roundStep)
        {
            int lowPart = number % roundStep;
            int highPart = number / roundStep;
            if (lowPart < roundStep / 4)
                lowPart = 0;
            else if (lowPart > roundStep * 3 / 4)
                lowPart = roundStep;
            else
                lowPart = roundStep / 2;
            return highPart * roundStep + lowPart;
        ...
        #region Long
        public static string DigitGrouping(this long number)
        {
            string sign = "";
            if (number < 0)
            {
